...
void TopicCommand::checkParams(Client* client,
    ...
    try {
        ...
        client->sendMessage(
            Replies::ERR_NOSUCHCHANNEL(client->getNickname(), chan->getName()));
        throw;
    }
    this->_channel = chan;
    if (!(chan->isInChannel(*client))) {
        client->sendMessage(
            Replies::ERR_NOTONCHANNEL(client->getNickname(), chan->getName()));
        throw;
    }
    if (params.size() > 1 && chan->getModifTopicByOps()) {
        if (!chan->isOperator(*client)) {
            client->sendMessage(Replies::ERR_CHANOPRIVSNEEDED(
                client->getNickname(), chan->getName()));
            throw;
        }
    }
}
...
void TopicCommand::run() {
    if (_params.size() < 2) {
        _client->sendMessage(Replies::RPL_TOPIC(
            _client->getName(), _channel->getName(), _channel->getTopic()));
    } else {
